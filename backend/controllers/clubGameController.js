...
exports.getAvailableGames = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(clubId) || clubId <= 0) {
      return res.status(400).json({ success: false, error: 'Invalid N999Bet club configuration' });
    }
    ...
    const result = games
      .map(game => extract(game, metaMap[`${game.aliases?.[0]?.prov}/${game.aliases?.[0]?.name}`]))
      .filter(Boolean);
    ...
    return res.json({ success: true, clubId, games: filtered });
  } catch (error) {
    ...
    return res.status(error.status || 502).json({
      ...
      code: error.code || 'GAME_CATALOG_UNAVAILABLE'
    });
  }
};
