...
exports.getSuperAdmins = async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT u.*, ab.balance FROM users u LEFT JOIN admin_balances ab ON u.id = ab.admin_id WHERE u.role = "super_admin"'
    );
    res.json({ success: true, superAdmins: rows });
  } catch (error) {
    console.error('Get super admins error:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch super admins' });
  }
};
...
exports.updateSuperAdmin = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Super admin updated' });
  } catch (error) {
...
exports.deleteSuperAdmin = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to delete super admin' });
  }
};

// ============================================================
// ALL ADMINS (FULL CONTROL)
// ============================================================
  ...
  try {
    const [rows] = await pool.query(
      `SELECT u.*, ab.balance 
       ...
       WHERE u.role IN ("super_admin", "admin", "employee") 
       ORDER BY FIELD(u.role, "super_admin", "admin", "employee")`
    );
...
exports.addBalanceToSuperAdmin = async (req, res) => {
  try {
    ...
    if (!mainBal || parseFloat(mainBal.balance) < amount) {
      return res.status(400).json({ success: false, error: 'Insufficient main admin balance' });
    }
    ...
    try {
      ...
      res.json({ success: true, message: 'Balance added to super admin' });
    } catch (err) {
    ...
    console.error('Add balance to super admin error:', error);
...
exports.addBalanceToAdmin = async (req, res) => {
  try {
    ...
    try {
      ...
      await connection.commit();
      res.json({ success: true, message: 'Balance added to admin' });
    } catch (err) {
      await connection.rollback();
      throw err;
    } finally {
...
exports.getBalanceOverview = async (req, res) => {
  try {
    const [adminBalances] = await pool.query(
      `SELECT u.id, u.username, u.role, ab.balance 
       FROM users u 
       LEFT JOIN admin_balances ab ON u.id = ab.admin_id 
       WHERE u.role IN ("main_admin", "super_admin", "admin", "employee")`
    );
    ...
    const [txStats] = await pool.query(
      `SELECT SUM(CASE WHEN type="deposit" AND status="completed" THEN amount ELSE 0 END) as total_deposits,
    ...
    res.json({
      ...
      overview: {
        adminBalances,
        userBalances: {
          main: parseFloat(userBalances[0]?.total_main || 0),
          bonus: parseFloat(userBalances[0]?.total_bonus || 0),
          commission: parseFloat(userBalances[0]?.total_commission || 0)
        },
        transactionVolume: {
    ...
    console.error('Get balance overview error:', error);
    res.status(500).json({ success: false, error: 'Failed to get balance overview' });
  }
};
  ...
  try {
    const [admins] = await pool.query(
      `SELECT u.id, u.username, u.role, 
       ...
       FROM users u 
       LEFT JOIN admin_activity_log al ON u.id = al.admin_id 
       WHERE u.role IN ("super_admin", "admin", "employee") 
       GROUP BY u.id`
    );
    ...
    res.status(500).json({ success: false, error: 'Failed to get admin audit' });
  }
};
...
exports.getTransactionAudit = async (req, res) => {
  try {
    ...
    const filter = period === 'today' ? 'DATE(created_at) = CURDATE()' :
                   period === 'week' ? 'created_at > DATE_SUB(NOW(), INTERVAL 7 DAY)' :
                   'created_at > DATE_SUB(NOW(), INTERVAL 30 DAY)';
    const [transactions] = await pool.query(
      `SELECT DATE(created_at) as date, 
              COUNT(*) as total, 
              SUM(CASE WHEN type="deposit" AND status="completed" THEN amount ELSE 0 END) as deposit_amount 
       FROM user_transactions WHERE ${filter} 
       GROUP BY DATE(created_at) ORDER BY date DESC`
    );
    res.json({ success: true, transactions });
  } catch (error) {
    console.error('Get transaction audit error:', error);
    res.status(500).json({ success: false, error: 'Failed to get transaction audit' });
  }
};
...
exports.getFullStats = async (req, res) => {
  try {
    ...
    const [financialStats] = await pool.query(
      'SELECT SUM(main_balance) as total_balance, SUM(bonus_balance) as total_bonus, SUM(commission_balance) as total_commission FROM wallets'
    );
    const [gameStats] = await pool.query(
      'SELECT COUNT(*) as total_games, SUM(total_plays) as total_plays FROM games'
    );
    const [adminBalanceStats] = await pool.query(
      'SELECT SUM(balance) as total_balance FROM admin_balances'
    );
    res.json({
      success: true,
      stats: {
        ...
        games: gameStats[0],
        adminBalance: adminBalanceStats[0]
      }
    });
  } catch (error) {
    console.error('Get full stats error:', error);
    res.status(500).json({ success: false, error: 'Failed to get full stats' });
  }
};
