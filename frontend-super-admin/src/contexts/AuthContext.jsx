...
const normalizeAuth = (data) => ({ token: data?.token || data?.access, refreshToken: data?.refreshToken, user: data?.user || null });
...
export const AuthProvider = ({ children }) => {
  ...
  const api = useMemo(() => {
    ...
    instance.interceptors.response.use((response) => response, async (error) => {
      ...
      if (error.response?.status !== 401 || original._retry || String(original.url || '').includes('/auth/refresh')) return Promise.reject(error);
      const refreshToken = localStorage.getItem('refreshToken');
      if (!refreshToken) { clearSession(); navigate('/login', { replace: true }); return Promise.reject(error); }
      original._retry = true;
      try {
        if (!refreshPromise.current) refreshPromise.current = axios.post(`${API_URL}/auth/refresh`, { refreshToken }, { timeout: 15000 }).then((response) => normalizeAuth(response.data)).finally(() => { refreshPromise.current = null; });
      ...
      } catch (refreshError) { clearSession(); navigate('/login', { replace: true }); return Promise.reject(refreshError); }
    });
    return instance;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (active && authGeneration.current === generation) { setUser(userData); localStorage.setItem('user', JSON.stringify(userData)); setIsAuthenticated(true); }
      } catch { if (active && authGeneration.current === generation) clearSession(); }
  ...
  const login = async (identifier, password) => {
    try {
      const response = await api.post('/auth/login', { identifier: String(identifier || '').trim(), password });
    ...
    } catch (error) { const message = error.response?.data?.error || error.message || 'Login failed.'; toast.error(message); throw new Error(message); }
  };

  const logout = useCallback(() => { clearSession(); toast.success('Signed out.'); navigate('/login', { replace: true }); }, [clearSession, navigate]);
