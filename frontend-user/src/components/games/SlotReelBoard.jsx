  ...
  list.forEach((win) => {
    ...
    if (Array.isArray(positions)) positions.forEach((p) => {
      if (Array.isArray(p) && p.length >= 2) {
        ...
        if (Number.isFinite(x) && Number.isFinite(y)) keys.add(`${Math.max(0, y - 1)}:${Math.max(0, x - 1)}`);
      } else if (p && typeof p === 'object') {
        const x = Number(p.x ?? p.col ?? p.column ?? 1); const y = Number(p.y ?? p.row ?? 1);
        if (Number.isFinite(x) && Number.isFinite(y)) keys.add(`${Math.max(0, y - 1)}:${Math.max(0, x - 1)}`);
      }
    });
    ...
    if (Array.isArray(line)) line.forEach((row, col) => {
      const r = Number(row);
      if (Number.isFinite(r)) keys.add(`${Math.max(0, r - 1)}:${col}`);
    });
  });
  return keys;
};
...
function SymbolArt({ value, provider = '', gameName = '' }) {
  ...
  return (
    <div className={`relative flex h-[76%] w-[76%] items-center justify-center overflow-hidden rounded-[28%] border border-white/15 bg-black/10 shadow-[inset_0_1px_12px_rgba(255,255,255,.08)] ${kind === 'wild' ? 'ring-2 ring-fuchsia-300/50' : ''} ${kind === 'scatter' ? 'ring-2 ring-amber-300/50' : ''} ${kind === 'bonus' ? 'ring-2 ring-emerald-300/50' : ''}`} style={{ color: `hsl(${hue} 88% 72%)` }} title={label}>
      ...
      <span className="relative z-10 max-w-[86%] truncate rounded-full bg-black/35 px-2 py-1 text-[clamp(8px,1.1vw,12px)] font-black uppercase tracking-[.12em] text-white/90 backdrop-blur-sm">{short}</span>
    </div>
  );
}
...
export default function SlotReelBoard({ grid, wins = [], spinning = false, sound = true, reels = 0, rows = 0, provider = '', gameName = '', quickSpin = false }) {
  ...
  useEffect(() => {
    ...
    const amount = list.reduce((sum, w) => sum + Number(w?.pay ?? w?.win ?? 0), 0);
  ...
  return (
    <div className="relative w-full max-w-6xl">
      ...
      <div className="relative slot-runtime-board overflow-hidden rounded-[32px] border border-white/10 p-2 sm:p-4 md:p-6">
        ...
        <div className="relative mb-3 flex items-center justify-between gap-3 rounded-xl border border-white/[.06] bg-black/20 px-3 py-2 text-[9px] uppercase tracking-[.22em] text-slate-400"><span className="truncate">{provider ? `${provider} · ` : ''}{title}</span><span className="shrink-0 text-amber-200">AUTHORITATIVE ENGINE</span></div>
        <div className={`relative grid gap-1.5 sm:gap-2 md:gap-3 ${spinning ? 'slot-spinning' : ''}`} style={{ gridTemplateColumns: `repeat(${Math.max(1, colCount)}, minmax(0, 1fr))` }}>
          {cells.map(({ value, r, c }) => {
            ...
            const kind = symbolKind(value);
            return <motion.div key={`${phase}-${key}`} initial={{ y: spinning ? -260 - c * 35 : 0, opacity: spinning ? 0.15 : 1, scaleY: spinning ? 1.3 : 1 }} animate={{ y: 0, opacity: 1, scaleY: 1, scale: isWin && settled ? 1.045 : 1 }} transition={{ duration, delay: spinning ? c * (quickSpin ? .025 : .075) : 0, ease: 'easeOut' }} onAnimationComplete={() => { if (sound && spinning && r === rowCount - 1) { slotAudio.reelStop(c); if (c === colCount - 1) setSettled(true); } }} className={`slot-runtime-cell relative flex aspect-square min-h-[58px] items-center justify-center overflow-hidden rounded-2xl border select-none sm:min-h-[84px] md:min-h-[112px] ${isWin && settled ? 'border-amber-300/90 shadow-[0_0_30px_rgba(251,191,36,.35)]' : 'border-white/10'} ${kind === 'wild' ? 'bg-fuchsia-950/80' : kind === 'scatter' ? 'bg-amber-950/80' : kind === 'bonus' ? 'bg-emerald-950/80' : ''}`}><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_22%,rgba(255,255,255,.16),transparent_42%)]" /><SymbolArt value={value} provider={provider} gameName={gameName} />{isWin && settled && <motion.span initial={{ opacity: 0, scale: .7 }} animate={{ opacity: [0, 1, .2], scale: [.75, 1.12, 1.35] }} transition={{ duration: .9, repeat: Infinity, repeatDelay: .35 }} className="pointer-events-none absolute inset-1 rounded-2xl border-2 border-amber-300" />}</motion.div>;
          })}
        </div>
        <div className="mt-3 flex items-center justify-between text-[9px] uppercase tracking-[.18em] text-slate-500 sm:text-[11px]"><span>{spinning ? 'Server spin in progress' : 'Authoritative server result'}</span><span>{colCount} × {rowCount}</span></div>
      </div>
