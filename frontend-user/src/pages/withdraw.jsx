...
const Withdraw = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (amt < 500) { toast.error('Minimum withdraw is 500 THB'); return; }
    if (amt > (balance?.main || 0)) { toast.error('Insufficient balance'); return; }
    if (!selectedBank) { toast.error('Select a bank account'); return; }
    setLoading(true);
    try {
      await requestWithdraw(amt, selectedBank);
      navigate('/wallet');
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-md">
      ...
      <div className="bg-dark-800 rounded-xl p-4 mb-6"><p className="text-gray-400 text-sm">Available Balance</p><p className="text-2xl font-bold text-white">{balance?.main?.toFixed(2) || '0.00'} THB</p></div>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-gray-300 text-sm font-medium mb-2">Amount (THB)</label>
          <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Enter amount" className="w-full px-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition" min="500" step="1" />
        </div>
        ...
        {bankAccounts.length > 0 ? (
          ...
          <div className="bg-dark-800 rounded-xl p-4 text-center">
            ...
            <button type="button" onClick={() => navigate('/wallet/banks')} className="mt-2 text-primary-500 text-sm hover:text-primary-400">+ Add Bank Account</button>
          </div>
        )}
        <button type="submit" disabled={loading || bankAccounts.length === 0} className="w-full py-3 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-500 transition disabled:opacity-50 flex items-center justify-center gap-2">
          {loading ? <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Processing...</> : <><FaWallet /> Submit Withdraw</>}
        </button>
        <div className="text-center text-xs text-gray-500"><p>⚠️ Withdrawals processed within 24 hours</p><p className="mt-1">Minimum withdraw: 500 THB</p></div>
      </form>
    </div>
  );
};
