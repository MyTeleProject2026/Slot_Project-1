    ...
    try {
      ...
      if (result && result.success !== false && sessionId) {
        setSession({
          ...
          slotopolGameId: result.session?.gid || result?.data?.session?.gid,
          gameState: result.session?.game || result?.data?.session?.game,
          wallet: result.wallet ?? result?.data?.wallet ?? 0,
        });
        setSpinResult(null);
